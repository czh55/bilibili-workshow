...
async function loadData() {
  const parts = [
    './b52_svg_data_part1.mjs',
    './b52_svg_data_part2.mjs',
    './b52_svg_data_part3.mjs',
    './b52_svg_data_part4.mjs',
    './b52_svg_data_part5.mjs',
    './b52_svg_data_part6.mjs',
    './b52_svg_data_part7.mjs',
    './b52_svg_data_part8.mjs',
    './b52_svg_data_part9.mjs',
    './b52_svg_data_part10.mjs',
    './b52_svg_data_part11.mjs',
    './b52_svg_data_part12.mjs',
    './b52_svg_data_part13.mjs',
    './b52_svg_data_part14.mjs',
    './b52_svg_data_part15.mjs',
    './b52_svg_data_part16.mjs',
    './b52_svg_data.mjs',
  ];
  const merged = {};
  for (const p of parts) {
    try {
      ...
      Object.assign(merged, mod.DATA || {});
    } catch {
      /* part may not exist yet */
    }
  }
  return merged;
}
...
.card.card-blue{border-left-color:#3b82f6}
...
function buildBody(d) {
  ...
  // 兼容 b50（table/boundary/conclusion对象）与 b51 part（actions/boundaries/conclusion字符串）
  const boundaryList = d.boundary || d.boundaries || [];
  const boundary = boundaryList.map((b) => `<li>${esc(b)}</li>`).join('');
  const pitfalls = (d.pitfalls || []).map((p) => `<div class="pitfall">${esc(p)}</div>`).join('');

  let tableBlock = '';
  if (d.table && d.table.head) {
  ...
  let actions = [];
  let shift = '';
  if (d.conclusion && typeof d.conclusion === 'object') {
    key = d.conclusion.key || [];
    actions = d.conclusion.actions || d.actions || [];
    shift = d.conclusion.shift || '';
  } else {
    actions = d.actions || [];
    shift = typeof d.conclusion === 'string' ? d.conclusion : '';
    key = (d.map || []).slice(0, 4).map(([label, x]) => `${label}：${x}`);
  }
  const keyHtml = key.map((k) => `<li>${esc(k)}</li>`).join('');
  const actionsHtml = actions.map((a) => `<li>${esc(a)}</li>`).join('');
  const footerDur = d.footer_duration || d.duration || '';

  return `<div class="container root-wrap">
  <h1>${esc(d.title)}</h1>
  <div class="meta">${tags}<span class="tag tag-gray">时长 ${esc(d.duration)}</span><span class="tag tag-gray">${esc(d.perspective || '理性分析')}</span></div>
  ...
  <div class="footer">双轨产物之二 · 理性分析 · 证据来自同一 Whisper 转录 · ${esc(footerDur)}</div>
</div>`;
}
