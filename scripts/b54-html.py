#!/usr/bin/env python3
"""b54：生成达芬奇调色批次图文实录 HTML。

用法：
  python3 scripts/b54-html.py {slug} [{slug} ...]
  python3 scripts/b54-html.py          # 合并 part1+part2+… 全部
"""
...
# key 前缀 → 本地 whisper 文件前缀；由 slug-map + 各 part data 的 key 字段决定
def load_data() -> dict:
    data: dict = {}
    for i in range(1, 20):
        try:
            mod = __import__(f"b54_data_part{i}")
            data.update(getattr(mod, "DATA", {}))
        except ImportError:
            pass
    try:
        mod = __import__("b54_data")
        data.update(getattr(mod, "DATA", {}))
    except ImportError:
        pass
    return data
...
def build_transcript(key: str) -> str:
    whisper = json.loads((ROOT / f"{key}.json").read_text(encoding="utf-8"))
    rows = []
...
def build_html(slug: str, d: dict) -> str:
    ...
    transcript = build_transcript(d["key"])
    ...
    for c in d.get("chapters", []):
        ...
        for num in c.get("figs", []):
            ...
            if not meta:
                continue
            # WORKFLOW：figcaption 单时间戳 [MM:SS] 中文图注，勿用 time-badge 重复
            scene = meta.get("caption") or meta.get("scene") or ""
            figs += (
                f'<figure><img src="assets/{slug}/shot-{num:02d}.jpg" '
                f'alt="{scene}" loading="lazy">'
                f'<figcaption>[{meta["time"]}] {scene}</figcaption></figure>'
            )
        story.append(
...
<header><h1>{d['title']}</h1>
<div class="meta-row"><span class="meta-tag tag-platform">{"B站" if d.get("platform") == "bilibili" else "小红书"}</span><span class="meta-tag tag-duration">{d['duration']}</span><span class="meta-tag tag-topic">{d['tags']}</span></div>
...
def main() -> None:
    data = load_data()
    if not data:
        raise SystemExit("未找到 b54_data_part*.py")
    args = sys.argv[1:]
