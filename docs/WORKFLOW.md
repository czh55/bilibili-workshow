...
### 视频时长分档（HTML/SVG/截图的最低标准）

不同长度视频的产出要求差异极大。处理前先按 Step 1 下载到的 `duration_string`（及转录后的非空 `segments` 数）确定时长档，然后按该档执行章节数、编辑字符量、配图数的最低标准：
...
- 每个 `figure` 必须有存在的本地图片、具体 `alt` 和含时间的图注；截图数量应等于证据表中可用截图数。配图数按「视频时长分档」表执行；仅在源资产缺失时允许低于该档下限，并必须在正文写明“本地截图缺失”。
- 检测到英文句、繁体字、`...`/`…`、占位标题 `XiaoHongShu video #`、连续乱码词或未完成句时，阻断发布并进入术语复核。
- HTML 的章节数与编辑字符量按「视频时长分档」表执行（如视觉型短视频 ≥1 章/200 字、短中视频 ≥3 章/800 字、长视频 ≥6 章/1500 字、超长视频 ≥8 章/2500 字）；任何长度都不得仅由转录拼接构成。
...
HTML 可以解释必要背景，但不应把叙事切碎成大量分析卡片。它回答的是：“视频具体说了什么、展示了什么、前后如何发展？”章节数量按「视频时长分档」表执行：视觉型短视频至少 1 个画面驱动章节，普通短视频至少 2 个，90 秒及以上至少 3 个，超长视频至少 8 个。若素材确实只有一个连续操作，按操作阶段拆分，并在文中说明没有明显段落切换。
...
SVG 的模块数量同样按「视频时长分档」执行：视觉型短视频（<30 秒或 `segments` ≤3）允许合并为 2–3 个模块（如「核心对比 + 关键结论」），避免空泛；长视频（>10 分钟）观点卡片不得少于 4 张；超长视频（>30 分钟）至少覆盖 5 个分析主题。核心命题、行动清单与避坑总结对任何长度都是必需模块，不得因短视频而省略。
...
- 配图数量按「视频时长分档」表执行（视觉型短视频 1–3 张、普通短视频 2–4 张、长视频 8–12 张、超长视频 12–20 张），每张对应一个内容节点；增强版可突破上限，见「完整版 / 增强版 HTML」
